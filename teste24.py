            ...
            if st.button("Processar Modelo Acadêmico"):
                ...
                for i, (nome, (X_data, y_data)) in enumerate(conjuntos.items()):
                    ...
                    with torch.no_grad():
                        ...
                        metrics = calcular_metricas(y_data.tolist(), y_pred)
                        resultados.append({"Conjunto": nome, **metrics})
                        axs[i].plot(y_data.tolist(), label="Real", alpha=0.7)
                ...
                st.subheader("Descrição Detalhada do Processo")
                st.write("""
                    1. **Configuração Inicial e Carregamento dos Dados**: Filtragem para 'AMST' e interpolação para valores ausentes.
                    2. **Normalização e Preparação dos Dados**: Escala com MinMaxScaler e divisão em conjuntos de treino, validação e teste.
                    3. **Treinamento do Modelo LSTM**: Usado para capturar correlações temporais, treinado com o número especificado de iterações.
                    4. **Avaliação e Visualização dos Resultados**: Avaliado em RMSE, MAE, R², AUC, Gini, KS e Assertividade; gráficos de comparação.
                """)

                st.subheader("Referência Acadêmica")
                st.write("Takahashi, M. B., Oliveira, H. C., Núñez, E. G. F., & Rocha, J. C. (2019). Brewing process optimization by artificial neural network and evolutionary algorithm approach. *Journal of Food Process Engineering*, 42, e13103. https://doi.org/10.1111/jfpe.13103")
